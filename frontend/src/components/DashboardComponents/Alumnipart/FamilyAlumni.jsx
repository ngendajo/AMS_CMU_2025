...
export default function FamilyAlumni() {
  ...
  useEffect(() =>{ /* 用 useEffect 钩子定义了一个副作用函数。副作用函数是在组件渲染完成后执行的函数 */
    ...
    const getalumniusers = async () =>{
        try{
            const response = await axios.get('http://127.0.0.1:8000/api/alumnilist/?family_id='+params.id,{ /* 用 axios 库发送了一个异步 GET 请求*/
                headers: { /* 请求头 */
  ...
  useEffect(() =>{ /* 用 useEffect 钩子定义了一个副作用函数。副作用函数是在组件渲染完成后执行的函数 */
    ...
    const getusers = async () =>{
        try{
            ...
            response.data.forEach(element => {
              if(element.alumn){
                if(parseInt(element.alumn.family.id)===parseInt(params.id)){
                  console.log(element)
                  alumnilist2.push({
                    no:i,
                    email:element.email,
