...
class AlumniBusinessViewSet(viewsets.ModelViewSet):
    ...
    def _prepare_data(self, request_data):
        """
        Prepare request data for processing
        """
        # Convert request data to dictionary if it's not already
        if hasattr(request_data, 'dict'):
            return request_data.dict()
        return request_data

    def _handle_errors(self, error, context=None):
        """
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return self._handle_errors(e, {
                'request_data': data,
                'error_details': e.detail if hasattr(e, 'detail') else {}
            })
        
        except Exception as e:
            return Response({
                'error': f'Unexpected error during business creation: {str(e)}',
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            return Response({
                'error': f'Unexpected error during business update: {str(e)}',
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            return Response({
                'error': f'Error retrieving businesses: {str(e)}',
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            return Response({
                'error': f'Error retrieving business: {str(e)}',
...
class StudentsPerGradeView(APIView):
    def get(self, request):
        try:
            # Annotate grades with student counts, filtering for current users
...
class AttendanceTakenViewSet(viewsets.ModelViewSet):
    ...
    def delete_absenteeism(self, request, pk=None):
        attendance_taken = self.get_object()
        absenteeism_id = request.data.get('absenteeism_id')
        delete_comments = request.data.get('delete_comments', True)  # Optional parameter

        if not absenteeism_id:
            return Response(
                {"error": "absenteeism_id is required"}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            # Remove from AttendanceTaken
            attendance_taken.absentees.remove(absenteeism)

            if delete_comments:
                # Get all associated comments
                comments = absenteeism.school_comments.all()
                # Delete all comments
            ...
            return Response({
                "message": "Absenteeism and associated data deleted successfully"
            }, status=status.HTTP_200_OK)
...
class EapAttendanceViewSet(viewsets.ModelViewSet):
    ...
    def update_eap_attendance(self, request,pk=None):
        try:
            ...
            student_id = absenteeism_data.get('absenteeism[student]')
            status_value = absenteeism_data.get('absenteeism[status]')
...
class ExamScheduleViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {'error': 'This time slot is already booked for this room on this date'},
                status=status.HTTP_400_BAD_REQUEST
            )
        except Exception as e:
            return Response(
