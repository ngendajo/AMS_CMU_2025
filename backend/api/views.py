...
class StudyReportView(APIView):
    # permission_classes = [IsAuthenticated, ]
    def get(self, request):
        stud = Studie.objects.raw(
            "select 1 as id, count(userprofile_studie.level) as level, userprofile_studie.level as degree from userprofile_studie group by userprofile_studie.level;")
